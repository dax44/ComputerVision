...
To prosty pomysł - ale jak się okazuje, bardzo proste mechanizmy, odpowiednio skalowane, mogą w końcu wyglądać jak magia `r emo::ji("hear_no_evil_monkey")`.

## Jak działa deep learning?

Wiemy już, że uczenie maszynowe polega na mapowaniu danych wejściowych (takich jak obrazy) na dane docelowe (takie jak etykieta "kot"), co odbywa się poprzez obserwację wielu przykładów danych wejściowych i danych docelowych.
...
Aby kontrolować wyjście sieci neuronowej, musimy być w stanie zmierzyć, jak daleko to wyjście jest od tego, czego się spodziewaliśmy.
Jest to zadanie dla funkcji straty sieci, zwanej również funkcją celu.
Funkcja straty bierze predykcje sieci oraz prawdziwy wynik i oblicza wynik odległości, ujmując, jak dobrze sieć poradziła sobie z tym konkretnym przykładem (patrz @fig-dl4).
...
Ta korekta jest zadaniem optymalizatora, który implementuje coś, co nazywa się algorytmem wstecznej propagacji (ang. *backpropagation*)[^deeplearning-1].
W dalszej części wyjaśnimy bardziej szczegółowo, jak działa wsteczna propagacja.

[^deeplearning-1]: główny algorytm w uczeniu głębokim
...
Sieć z minimalną stratą to taka, dla której wyjścia są tak bliskie celom, jak to tylko możliwe[^deeplearning-2].

[^deeplearning-2]: sieć wytrenowana
...
Następnie, w 2012 roku, zespół kierowany przez Alexa Krizhevsky'ego i wspierany przez Geoffreya Hintona był w stanie osiągnąć dokładność pierwszej piątki[^deeplearning-3] na poziomie 83,6% - był to znaczący przełom.
...
[^deeplearning-3]: (ang. *top 5 accuracy*) *-* oznacza, że wśród 5 kategorii z najwyższym prawdopodobieństwem jest prawdziwa klasa
...
W związku z tym, ludzie musieli zadać sobie wiele trudu, aby uczynić początkowe dane wejściowe bardziej podatnymi na przetwarzanie przez te metody: to znaczy, musieli ręcznie opracować dobre warstwy reprezentacji dla swoich danych.
Nazywa się to inżynierią cech.
Uczenie głębokie całkowicie automatyzuje ten krok: w przypadku uczenia głębokiego, uczy się wszystkich cech w jednym przejściu i nie musimy ich samodzielnie opracowywać.
...
To, co jest przełomowe w uczeniu głębokim, to fakt, że pozwala ono modelowi uczyć się wszystkich warstw reprezentacji wspólnie, w tym samym czasie.
...
Jest to znacznie bardziej efektywne niż składanie płytkich modeli, ponieważ pozwala na uczenie się złożonych, abstrakcyjnych reprezentacji poprzez rozbicie ich na długie serie pośrednich warstw; każda warstwa jest tylko prostym przekształceniem w stosunku do poprzedniej.

Dwie zasadnicze cechy tego, jak głębokie uczenie uczy się z danych, to: przyrostowy, warstwa po warstwie sposób, w jaki korygowane są coraz bardziej złożone reprezentacje, oraz fakt, że te pośrednie, przyrostowe reprezentacje są uczone wspólnie, a każda warstwa jest aktualizowana, aby podążać zarówno za potrzebami reprezentacyjnymi warstwy powyżej, jak i potrzebami warstwy poniżej.
...
Świetnym sposobem na poznanie aktualnego bogactwa algorytmów i narzędzi uczenia maszynowego jest przyjrzenie się konkursom uczenia maszynowego na Kaggle.
...
Należy nadmienić, że *gradient boosting* jest używany głównie do problemów, w których dostępne są ustrukturyzowane dane, podczas gdy głębokie uczenie jest używane do problemów percepcyjnych, takich jak klasyfikacja obrazów.
...
Jeśli chodzi o dane, to oprócz wykładniczego postępu w dziedzinie sprzętu do przechowywania danych w ciągu ostatnich 20 lat (zgodnie z prawem Moore'a[^deeplearning-4]), kluczowym czynnikiem był rozwój Internetu, dzięki któremu możliwe stało się gromadzenie i rozpowszechnianie bardzo dużych zbiorów danych na potrzeby uczenia maszynowego.
...
[^deeplearning-4]: mówi o tym, że liczba tranzystorów w procesorach rośnie wykładniczo
...
Oprócz sprzętu i danych, aż do późnych lat 2000 brakowało niezawodnego sposobu trenowania bardzo głębokich sieci neuronowych.
W rezultacie sieci neuronowe były wciąż dość płytkie, wykorzystując tylko jedną lub dwie warstwy reprezentacji; nie były więc w stanie zabłysnąć w porównaniu z bardziej wyrafinowanymi klasycznymi metodami, takimi jak SVM czy lasy losowe.
...
Dopiero gdy te ulepszenia pozwoliły na trenowanie modeli z 10 lub więcej warstwami, uczenie głębokie zaczęło błyszczeć.
...
Pamiętajmy, że pojęcia te będą niezbędne do zrozumienia praktycznych przykładów, które pojawią się w kolejnych rozdziałach!
...
Możemy myśleć o "rozwiązywaniu" MNIST jako o "Hello World" głębokiego uczenia - to jest to, co robimy, aby zweryfikować, że nasze algorytmy działają zgodnie z oczekiwaniami.
Gdy staniemy się praktykami uczenia maszynowego, zobaczymy, że MNIST pojawia się raz za razem, w pracach naukowych, wpisach na blogach i tak dalej.
...
Zauważmy, że funkcja `compile()` modyfikuje sieć na bieżąco (zamiast zwracać nowy obiekt sieci, co jest bardziej typowe dla R).
...
Zauważmy, że używamy funkcji `array_reshape()` zamiast funkcji `dim()` do zmiany kształtu tablicy.
...
Na tym kończymy nasz pierwszy przykład - właśnie zobaczyliśmy, jak można zbudować i wytrenować sieć neuronową do klasyfikacji pisma ręcznego w mniej niż 20 liniach kodu R.
W następnym rozdziale omówimy szczegółowo każdy element, który użyliśmy i wyjaśnimy, co on robi.
Dowiemy się o tensorach, obiektach przechowujących dane w sieci; o operacjach na tensorach, z których składają się warstwy oraz o spadku gradientu, który pozwala sieci uczyć się na przykładach treningowych.
...
Tensory są uogólnieniem wektorów i macierzy na dowolną liczbę wymiarów (zauważmy, że w kontekście tensorów "wymiar" jest często nazywany "osią").
W R wektory są używane do tworzenia i manipulowania tensorami 1D, a macierze są używane do tensorów 2D.
Dla wymiarów wyższego rzędu używane są obiekty tablicowe[^deeplearning-5] (które obsługują dowolną liczbę wymiarów).
...
-   Typ danych - typ danych zawartych w tensorze; na przykład, typem tensora może być liczba całkowita lub rzeczywista[^deeplearning-6]. W rzadkich przypadkach możesz zobaczyć tensor znakowy[^deeplearning-7]. Ponieważ jednak tensory żyją we wstępnie przydzielonych segmentach pamięci, a łańcuchy znaków, będąc zmiennej długości, wykluczyłyby użycie tej implementacji, są one rzadziej używane.

[^deeplearning-6]: float - czyli liczba rzeczywista

[^deeplearning-7]: typu `character`
...
Generalnie, możemy wybrać pomiędzy dwoma dowolnymi indeksami wzdłuż każdej osi tensora.
Na przykład, aby wybrać 14 × 14 pikseli w prawym dolnym rogu wszystkich obrazów, użyjemy:

```{r}
my_slice <- train_images[, 15:28, 15:28]
```

Generalnie, pierwszą osią we wszystkich tensorach danych, z którymi zetkniemy się w głębokim uczeniu, będzie oś próbek (czasami nazywana wymiarem próbek).
...
Jest to termin, który często spotkamy podczas korzystania z `keras` i innych bibliotek uczenia głębokiego.

Uściślijmy tensory danych za pomocą kilku przykładów podobnych do tego, co napotkamy później.
Dane, którymi będziemy manipulować, prawie zawsze będą należały do jednej z następujących kategorii:
...
Gdy w danych ważny staje się czas (lub kolejność sekwencji), sensowne jest przechowywanie ich w tensorze 3D z wyraźną osią czasu.
...
Tymczasem Theano umieszcza oś głębi koloru zaraz po osi partii: `(sample, color_depth, height, width)`.
Keras zapewniają wsparcie dla obu formatów.

Dane wideo są jednym z niewielu typów danych ze świata rzeczywistego, dla których będziemy potrzebować tensorów 5D.
...
To w sumie 106168320 liczb!
Jeśli typ danych tensora to *double*, to każda wartość jest przechowywana w 64 bitach, więc tensor reprezentowałby 810 MB.
Sporo `r emo::ji("fear")`!
Filmy, które spotykamy w prawdziwym życiu są znacznie lżejsze, ponieważ nie są przechowywane w `float32` i zazwyczaj są silnie skompresowane (jak w formacie MPEG).
...
Jak widzieliśmy wcześniej, każda warstwa neuronowa z naszego pierwszego przykładu sieci przekształca swoje dane wejściowe w następujący sposób:
...
1.  Wylosuj partię próbek treningowych `x` i odpowiadające im odpowiedzi `y`.
...
Chociaż możliwe byłoby rozwiązanie takiego równania dla $N = 2$ lub $N = 3$, to zrobienie tego jest niepraktyczne dla rzeczywistych sieci neuronowych, gdzie liczba parametrów nigdy nie jest mniejsza niż kilka tysięcy, a często może wynosić kilkadziesiąt milionów.
...
Zauważmy, że istnieje wariant algorytmu mini-batch SGD polegający na losowaniu pojedynczej próbki i `y` w każdej iteracji, zamiast losowania partii danych.
Jest on oryginalnym algorytmem SGD, który jest mniej wydajny.
...
Oznacza to, że biorąc pod uwagę łańcuch operacji ze znaną funkcją straty, mogą obliczyć funkcję gradientu dla łańcucha za pomocą pochodnej funkcji złożonej, która mapuje wartości parametrów sieci do wartości gradientu.
Kiedy mamy dostęp do takiej funkcji, przejście wsteczne jest zredukowane do wywołania tej funkcji gradientu.
Dzięki symbolicznemu różniczkowaniu nigdy nie będziemy musieli ręcznie implementować algorytmu wstecznej propagacji.

### Funkcje straty

We wcześniej prezentowanym przykładzie w funkcji `compile()` pojawiła się funkcja straty `loss = 'categorical_crossentropy'`, która jest jedną z możliwych do zastosowania funkcji straty[^deeplearning-8].
Funkcja straty jest używana jako sygnał zwrotny do uczenia tensorów wag i którą w fazie uczenia będzie minimalizowana.
...
[^deeplearning-8]: o funkcjach straty będziemy jeszcze wspominać przy konkretnych przykładach
