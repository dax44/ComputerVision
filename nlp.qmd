...
::: {#exm-1}
Najpierw ze strony [ai.stanford.edu/\~amaas/data/sentiment](ai.stanford.edu/~amaas/data/sentiment) pobierzemy surowy zbiór danych IMDB[^nlp-1].
Rozkompresuj go.
...
Na przykład, używając kodowania binarnego (multi-hot[^nlp-2]), zakodowałbyś tekst jako wektor o tylu wymiarach, ile jest słów w twoim słowniku, z 0 prawie wszędzie i kilkoma 1 dla wymiarów, które kodują słowa obecne w tekście.
...
[^nlp-1]: jeśli adres URL już nie działa, wygoogluj "IMDB dataset"

[^nlp-2]: kodowanie *multi-hot* jest kodowaniem potrzebującym nieco mniejszych wektorów do zakodowania słów, np.
