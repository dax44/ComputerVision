...
Jednak modelowanie świata wizualnego w całej jego bogatej złożoności jest o wiele trudniejsze niż, powiedzmy, modelowanie dzwięku, który wytwarzamy podczas mówienia.
...
-   Śledzenie ruchu: wykorzystywanie znaczników retrorefleksyjnych oglądanych z wielu kamer lub innych technik opartych na wizji w celu uchwycenia aktorów na potrzeby animacji komputerowej;
...
-   *Morphing*: przekształcanie zdjęcia jednego z przyjaciół w inne, przy użyciu płynnego przejścia morficznego;
...
W tym rozdziale przedstawiam krótkie, subiektywne streszczenie głównych osiągnięć w dziedzinie wizyjności komputerowej w ciągu ostatnich pięćdziesięciu lat, koncentrując się na postępach, które uważam za osobiście interesujące i które przetrwały próbę czasu.
...
W tym okresie prowadzono również badania nad lepszym wykrywaniem krawędzi i konturów [@cannyComputationalApproachEdge1986; @nalwaDetectingEdges1986], stereografii fotometrycznej [@woodhamAnalysingImagesCurved1981] oraz kształty z tekstur [@witkinRecoveringSurfaceShape1981].
W tym okresie prowadzono również badania nad lepszym wykrywaniem krawędzi i konturów, w tym wprowadzono dynamicznie ewoluujące trackery konturów, takie jak węże, a także trójwymiarowe modele oparte na fizyce.
...
Nagły wzrost aktywności w zakresie wykorzystania niezmienników projekcyjnych do celów rozpoznania [@mundyGeometricInvarianceComputer1992] przerodził się w skoordynowane wysiłki zmierzające do rozwiązania problemu *structure from motion*.
Wiele początkowych działań skierowanych było na rekonstrukcje rzutowe, które nie wymagają znajomości kalibracji kamery.
Równolegle, techniki faktoryzacji zostały opracowane w celu efektywnego rozwiązywania problemów, dla których miały zastosowanie przybliżenia kamery ortograficznej, a następnie rozszerzone na przypadek perspektywiczny.

W końcu zaczęto stosować pełną optymalizację globalną która później została uznana za tożsama z technikami dopasowania wiązki, tradycyjnie stosowanymi w fotogrametrii.
...
Algorytmy śledzenia również uległy dużej poprawie, w tym śledzenie konturów z wykorzystaniem aktywnych konturów, takich jak węże [@kassSnakesActiveContour1988], filtry cząsteczkowe [@blakeActiveContoursApplication2012] i zbiorów poziomnicowych (ang. *level set*) [@malladiShapeModelingFront1995], a także techniki oparte na intensywności (bezpośrednie), często stosowane do śledzenia twarzy.
...
I chociaż cechy oparte na punktach zainteresowania (patch-based) dominują w obecnych badaniach, niektóre grupy zajmują się rozpoznawaniem na podstawie konturów i segmentacji regionów.
...
Chociaż trend ten rozpoczął się od prac nad cięciami grafów, duży postęp dokonał się również w algorytmach przekazywania wiadomości, takich jak loopy belief propagation (LBP).
